...
| | |
|---|---|
| Elasticsearch writes | impossible while `readOnly: true` (default): the core has one HTTP call site and refuses anything but GET/HEAD and `_search`-family POSTs before opening a socket |
...
- Leave `readOnly: true`. The REST console's write ability is an operator decision that
  belongs in the file, not a button.
